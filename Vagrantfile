...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    curl -s http://download.draios.com/stable/deb/draios.list | sudo tee /etc/apt/sources.list.d/draios.list
    sudo apt-get update
    sudo apt-get install -y docker-engine linux-headers-$(uname -r) sysdig falco
